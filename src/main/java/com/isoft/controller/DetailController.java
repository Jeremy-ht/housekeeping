...
public class DetailController {
     ...
     * 发布
     */
    @GetMapping("/pullScenery/{id}")
    public ResponseData pullScenery(@PathVariable("id") Integer id) {
        if (id == null) {
            return ResponseData.error().message("请求参数不能为空");
        }

        return detailService.pullScenery(id) ? ResponseData.success().message("发布成功！")
